...
This capability owns the **byte-level wire contract**. All field names below MUST
appear verbatim (case-sensitive) in serialized JSON, matching `docs/protocol.md`.
The verifier byte-matches these.

I/O is forbidden in this layer (per `.harness/project.md` `placement_rules`); types
are pure `serde::Serialize + serde::Deserialize` data definitions.
